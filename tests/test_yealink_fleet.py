...
MODULE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "yealink_fleet.py"
SPEC = importlib.util.spec_from_file_location("yealink_fleet_script", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
YEALINK_FLEET = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = YEALINK_FLEET
SPEC.loader.exec_module(YEALINK_FLEET)

PageParser = YEALINK_FLEET.PageParser
aes_encrypt_password = YEALINK_FLEET.aes_encrypt_password
contexts_for = YEALINK_FLEET.contexts_for
extract_rsa_key = YEALINK_FLEET.extract_rsa_key
normalize_fingerprint = YEALINK_FLEET.normalize_fingerprint
redact_context = YEALINK_FLEET.redact_context
rsa_encrypt_text = YEALINK_FLEET.rsa_encrypt_text
